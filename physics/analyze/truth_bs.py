#!/usr/bin/env python3
"""Bs -> mu+mu- truth study analysis: load cheated Parquet, plot signal distributions."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Bs -> mu+mu- truth analysis (from Parquet)",
    )
    ...
    # Build observable arrays from DataFrame
    obs_values = {}
    ...
    obs_values["doca"] = sig[doca_cols].max(axis=1).values if doca_cols else np.zeros(n_sig)
    obs_values["dira"] = sig["dira"].fillna(0.0).values
    obs_values["B_pt"] = sig["pair_pt"].values
    obs_values["B_ip"] = sig["composite_min_ip"].fillna(0.0).values
    obs_values["B_ip_chi2"] = sig["composite_min_ip_chi2"].fillna(0.0).values
    ...
    fig.suptitle(
        f"$B_s^0 \\to \\mu^+\\mu^-$ cheated signal{lumi_tag} "
        f"({n_sig} candidates, {n_events} events)",
        fontsize=14,
    )
    fig.tight_layout()
    fig.savefig(out_dir / "bs_signal_distributions.png", dpi=150)
    print(f"\nSaved {out_dir / 'bs_signal_distributions.png'}")
