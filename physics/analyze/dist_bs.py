...
def main():
    ...
    def _build_obs_arrays(subset):
        ...
        obs["B_pt"] = subset["pair_pt"].values
        obs["B_ip"] = subset["composite_min_ip"].fillna(0.0).values
        obs["B_ip_chi2"] = subset["composite_min_ip_chi2"].fillna(0.0).values
        if len(ip_cols) >= 2:
    ...
    fig.suptitle(f"Bs signal vs background (normalised){lumi_tag}", fontsize=14)
    fig.tight_layout()
