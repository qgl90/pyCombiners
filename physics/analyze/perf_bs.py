#!/usr/bin/env python3
"""Bs -> mu+mu- performance analysis: load Parquet, apply post-cuts, plot efficiency and mass."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Bs -> mu+mu- performance analysis (from Parquet)",
    )
    ...
    # ---- Post-combination cuts on full candidates ----
    sel = full.copy()
    ...
    fig.suptitle(
        f"$B_s^0 \\to \\mu^+\\mu^-$ efficiency{lumi_tag} "
