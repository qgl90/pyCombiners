...
def main():
    ...
    def _build_obs_arrays(subset):
        ...
        obs["Ks_pt"] = subset["pt"].values
        obs["Ks_ip"] = subset["composite_ip"].fillna(0.0).values
        obs["Ks_ip_chi2"] = subset["composite_ip_chi2"].fillna(0.0).values
    ...
    fig.suptitle(f"Ks signal vs background (normalised){lumi_tag}")
    fig.tight_layout()
